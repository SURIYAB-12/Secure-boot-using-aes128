...
function [7:0] mb0e; // Multiply by 0x0e
  input [7:0] x;
  begin
    mb0e = gf_mul(x, 8'h0e);
  end
...
function [7:0] mb0b; // Multiply by 0x0b
  input [7:0] x;
  begin
    mb0b = gf_mul(x, 8'h0b);
  end
...
function [7:0] mb0d; // Multiply by 0x0d
  input [7:0] x;
  begin
    mb0d = gf_mul(x, 8'h0d);
  end
...
function [7:0] mb09; // Multiply by 0x09
  input [7:0] x;
  begin
    mb09 = gf_mul(x, 8'h09);
  end
...
generate
  for (i = 0; i < 4; i = i + 1) begin : m_col
    assign state_out[(i*32 + 24)+:8] = mb0e(state_in[(i*32 + 24)+:8]) ^ mb0b(state_in[(i*32 + 16)+:8]) ^
                                       mb0d(state_in[(i*32 + 8)+:8])  ^ mb09(state_in[i*32+:8]);

    assign state_out[(i*32 + 16)+:8] = mb09(state_in[(i*32 + 24)+:8]) ^ mb0e(state_in[(i*32 + 16)+:8]) ^
                                       mb0b(state_in[(i*32 + 8)+:8])  ^ mb0d(state_in[i*32+:8]);

    assign state_out[(i*32 + 8)+:8]  = mb0d(state_in[(i*32 + 24)+:8]) ^ mb09(state_in[(i*32 + 16)+:8]) ^
                                       mb0e(state_in[(i*32 + 8)+:8])  ^ mb0b(state_in[i*32+:8]);

    assign state_out[i*32+:8]        = mb0b(state_in[(i*32 + 24)+:8]) ^ mb0d(state_in[(i*32 + 16)+:8]) ^
...
// Composite Field Arithmetic Based Multiplication by 2
function [7:0] cfa_mb2;
  input [7:0] a;
  begin
    cfa_mb2 = {a[6:0], 1'b0} ^ (8'h1b & {8{a[7]}});
  end
endfunction

// Composite Field Arithmetic Based Multiplication by 3 = mb2(a) ^ a
function [7:0] cfa_mb3;
  input [7:0] a;
  begin
    cfa_mb3 = cfa_mb2(a) ^ a;
  end
...
generate
  for (i = 0; i < 4; i = i + 1) begin : m_col
    ...
    assign state_out[i*32 + 24 +: 8] = cfa_mb2(s3) ^ cfa_mb3(s2) ^ s1 ^ s0;
    assign state_out[i*32 + 16 +: 8] = s3 ^ cfa_mb2(s2) ^ cfa_mb3(s1) ^ s0;
    assign state_out[i*32 + 8  +: 8] = s3 ^ s2 ^ cfa_mb2(s1) ^ cfa_mb3(s0);
    assign state_out[i*32 + 0  +: 8] = cfa_mb3(s3) ^ s2 ^ s1 ^ cfa_mb2(s0);
  end
endgenerate
